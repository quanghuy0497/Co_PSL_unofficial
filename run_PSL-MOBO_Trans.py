...
from model import ParetoSetModel_Transformer as ParetoSetModel
...
for test_ins in ins_list:
    ...
    for i_iter in range(n_iter):
        ...
        with open(f"logs_{suffix_dir}/hv_{test_ins}_{n_dim}{suffix}.pkl", 'wb') as output_file:
        ...
        np.save(f"logs_{suffix_dir}/evaluation_{test_ins}_X_{n_dim}{suffix}", X)
        np.save(f"logs_{suffix_dir}/evaluation_{test_ins}_Y_{n_dim}{suffix}", Y)
        
        np.save(f"logs_{suffix_dir}/front_{test_ins}_{n_dim}{suffix}", front_list)
        np.save(f"logs_{suffix_dir}/x_{test_ins}_{n_dim}{suffix}", x_list)
        np.save(f"logs_{suffix_dir}/y_{test_ins}_{n_dim}{suffix}", y_list)