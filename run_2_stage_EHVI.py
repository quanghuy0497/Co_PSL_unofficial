...
hv_list = {}
n_evaluation = [220, 120]
for evaluation in n_evaluation:
    for test_ins in ins_list:
        
        suffix="_warmup_init_EHVI" + "_" + str(evaluation) + "_uniform"
        suffix_dir = ""
            ...
            if i_iter > 0:
                with torch.no_grad():
                    for params in psmodel.parameters():
                        xavier = torch.empty(params.shape).to(device)
                        if xavier.dim() > 1:
                            nn.init.xavier_uniform_(xavier)
                            params.mul_(beta).add_(xavier, alpha = 1-beta)
            if ((i_iter + 1) %10) == 0:
                beta *= 0.1
