...
class RE33():       # Disk Brake
    ...
    def evaluate(self, x):
        ...
        g = torch.stack([g1,g2,g3,g4]).to(device)
        z = torch.zeros(g.shape).float().cuda().to(torch.float64).to(device)
        g = torch.where(g < 0, -g, z)
...
class LSH():           # Rocket Injector
    def __init__(self, n_dim = 3):
        ...
        self.n_obj = 3
        self.bound = 0
        self.lbound = torch.tensor([0.0, 0, 0, 0]).float()
        self.ubound = torch.tensor([1, 1, 1, 1]).float()
        self.time_bound = torch.tensor([0.0, 0.65]).float()
        self.precision_bound = torch.tensor([0.0, 0.34]).float()
        self.memory_bound = torch.tensor([0, 1600]).float()
        
    def evaluate(self, x):
        
        
        dataset_file = 'dataset/glove.6B.300d.npy'
