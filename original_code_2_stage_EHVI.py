...
hv_list = {}
n_evaluation = [120, 220]
for evaluation in n_evaluation:
    for test_ins in ins_list:
        
        suffix="_warmup_init_EHVI" + "_" + str(evaluation)
        suffix_dir = "_originalPSL"
        
        if not os.path.exists(f"logs_{test_ins}{suffix_dir}"):
            os.makedirs(f"logs_{test_ins}{suffix_dir}")
            
        n_sample = 5 if evaluation == 120 else 5
        ...
        front_list = np.zeros((n_iter, n_candidate, n_obj))
        x_list = np.zeros((n_iter, n_candidate, n_dim))
        y_list = np.zeros((n_iter, n_candidate, n_obj))
        ...
        x_init = np.load(f"logs_warmup/EHVI_{test_ins}_X_{n_dim}_{evaluation}.npy")
        ...
        for i_iter in range(n_iter):
            ...
            # $$\theta_i = (1-\beta)*\theta_{i-1} + beta * \mathcal{N}
            if i_iter > 0:
                with torch.no_grad():
                    for params in psmodel.parameters():
                        gauss = torch.from_numpy(np.random.normal(size=params.size())).to(device)
                        params.mul_(1 - beta).add_(gauss, alpha = beta)
            if ((i_iter + 1) %10) == 0:
                beta *= 0.1
